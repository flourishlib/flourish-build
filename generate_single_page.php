...
if (!$tag) {
	echo "ERROR: No tag present on current commit\n";
	exit(1);
}

$skip_regex = '#^(CodeStandards|CompilingPHPOnWindows|ConstructorFunctions|ContributorLicenseAgreements|MotivationsForFlourish|Roadmap|ServerFund|ServerRequirements|SitesPoweredByFlourish|index)#';
$other_docs = array();
$classes    = array();
$files      = array_diff(scandir($wiki_docs_dir), array('.', '..'));
foreach ($files as $file) {
	$file = str_replace('.html', '', $file);
	if ($file[0] != 'f') {
		if (preg_match($skip_regex, $file)) {
			continue;
		}
		$other_docs[] = $file;
	} else {
...
foreach ($classes as $class) {	
	$api_html  = file_get_contents($api_docs_dir . $class . '.html');
	$wiki_html = file_get_contents($wiki_docs_dir . $class . '.html');

	$api_html = preg_replace('#^.*</header>#si', '', $api_html);
	$api_html = preg_replace('#<footer>.*$#si', '', $api_html);
	$api_html = str_replace('<script src="/js/api.js"></script>', '', $api_html);

	$wiki_html = preg_replace('#^.*</header>#si', '', $wiki_html);
	$wiki_html = preg_replace('#<footer>.*$#si', '', $wiki_html);
	
	// Move headings down 2 levels
	foreach (array(4, 3, 2, 1) as $level) {
		$api_html  = str_replace('<h' . $level, '<h' . ($level+2), $api_html);
		$wiki_html = str_replace('<h' . $level, '<h' . ($level+1), $wiki_html);
		$api_html  = str_replace('</h' . $level, '</h' . ($level+2), $api_html);
		$wiki_html = str_replace('</h' . $level, '</h' . ($level+1), $wiki_html);
	}
	
	// API DOCS
	$api_html = preg_replace('#^<a name="top"></a>#', '', $api_html);
	$api_html = preg_replace('#<li>\s*<a( class="source")? href="https://github.com/flourishlib/flourish-classes/blob/master/' . $class . '.php">Source Code</a>\s*</li>#', '', $api_html);
	
	$api_html = str_replace('id="', 'id="' . $class . '-api-', $api_html);
	$api_html = preg_replace('#<h3([^>]*>f[a-zA-Z0-9]+)#', '<h3 class="api" id="' . $class . '-api"\1 API Reference', $api_html);
	
	$api_html = str_replace('href="#', 'href="#' . $class . '-api-', $api_html);
	
	// Rewrite method references directly to the method section
	$api_html = preg_replace('/href="(f[a-zA-Z0-9]+)#([^"]+)"/', 'href="#\1-api-\2"', $api_html);
	
	// Rewrite a class API reference to the main class section
	$api_html = preg_replace('/href="(f[^"]+)"/', 'href="#\1"', $api_html);
	
	// Rewrite class references to the main class section
	$api_html = preg_replace('@href="(?:http://flourishlib\.com)?/(f[^"]+)"@', 'href="#\1"', $api_html);
	$api_html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[^"]+)"@', 'href="#\1"', $api_html);
	$api_html = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[^"]+)"@', 'href="#\1-api"', $api_html);
	// Rewrite all other docs references to the doc's section
	$api_html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(?!About|Blog|CodeStandards|ContributorLicenseAgreements|Documentation|Download|MotivationsForFlourish|Roadmap|ServerFund|ServerRequirements|SitesPoweredByFlourish|Support|Tests|Welcome)([^"]+)"@', 'href="#\1-docs"', $api_html);
	// Rewrite content referencing the full flourish site that is available in this document
	$api_html = preg_replace('@http://flourishlib\.com/docs/@', '', $api_html);
	
	
	// WIKI DOCS
	$wiki_html = preg_replace('#<li>\s*<a( class="source")? href="https://github.com/flourishlib/flourish-classes/blob/master/' . $class . '.php">Source Code</a>\s*</li>#', '', $wiki_html);
	$wiki_html = str_replace('id="', 'id="' . $class . '-docs-', $wiki_html);
	$wiki_html = preg_replace('#<h2.*?</h2>#', '<h2 id="' . $class . '">' . $class . '</h2>', $wiki_html);
	
	$wiki_html = str_replace('href="#', 'href="#' . $class . '-docs-', $wiki_html);
	
	// Rewrite class references to the main class section
	$wiki_html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-docs-\2"', $wiki_html);
	$wiki_html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[^"]+)"@', 'href="#\1"', $wiki_html);
	$wiki_html = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-api-\2"', $wiki_html);
	$wiki_html = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[^"]+)"@', 'href="#\1-api"', $wiki_html);
	// Rewrite all other docs references to the doc's section
	$wiki_html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(?!About|Blog|CodeStandards|ContributorLicenseAgreements|Documentation|Download|MotivationsForFlourish|Roadmap|ServerFund|ServerRequirements|SitesPoweredByFlourish|Support|Tests|Welcome)([^"]+)"@', 'href="#\1-docs"', $wiki_html);
	// Rewrite content referencing the full flourish site that is available in this document
	$wiki_html = preg_replace('@http://flourishlib\.com/docs/@', '', $wiki_html);
	
	
	$output .= $wiki_html . '<div class="docs">' . $api_html . '</div>';
}


$titles = array();
foreach ($other_docs as $file) {
	$html = file_get_contents($wiki_docs_dir . $file . '.html');

	$html = preg_replace('#^.*</header>#s', '', $html);
	$html = preg_replace('#<footer>.*$#s', '', $html);

	preg_match('#<h1.*?>(.*?)</h1>#', $html, $title_match);
	$titles[$file] = strip_tags($title_match[1]);
	
	// Move headings down 2 levels
	foreach (array(4, 3, 2, 1) as $level) {
		$html = str_replace('<h' . $level, '<h' . ($level+1), $html);
		$html = str_replace('</h' . $level, '</h' . ($level+1), $html);
	}
	
	// WIKI DOCS
	$html = str_replace('id="', 'id="' . $file . '-docs-', $html);
	$html = preg_replace('#<h2[^>]*>(.*?)</h2>#', '<h2 id="' . $file . '-docs">\1</h2>', $html);
	
	$html = str_replace('href="#', 'href="#' . $file . '-docs-', $html);
	
	// Rewrite class references to the main class section
	$html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-docs-\2"', $html);
	$html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[^"]+)"@', 'href="#\1"', $html);
	$html = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-api-\2"', $html);
	$html = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[^"]+)"@', 'href="#\1-api"', $html);
	// Rewrite all other docs references to the doc's section
	$html = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(?!About|Blog|CodeStandards|ContributorLicenseAgreements|Documentation|Download|MotivationsForFlourish|Roadmap|ServerFund|ServerRequirements|SitesPoweredByFlourish|Support|Tests|Welcome)([^"]+)"@', 'href="#\1-docs"', $html);
	// Rewrite content referencing the full flourish site that is available in this document
	$html = preg_replace('@http://flourishlib\.com/docs/@', '', $html);
	$html = str_replace('href="/', 'href="http://flourishlib.com/', $html);
	
	$output .= $html;
}


$documentation_html = file_get_contents($wiki_docs_dir . 'index.html');

$documentation_html = preg_replace('#^.*</header>#s', '', $documentation_html);
$documentation_html = preg_replace('#<footer>.*$#s', '', $documentation_html);

preg_match('#<h2 id="ClassDocumentation">Class Documentation</h2>(.*?)(?=<h2 id="GeneralDocumentation)#s', $documentation_html, $match);
$match[1] = preg_replace('#,\s+<a href="https://github.com/flourishlib/flourish-classes/blob/master/.*?</a>#', '', $match[1]);
$match[1] = preg_replace('#<(/?)h3>#', '<\1h4>', $match[1]);
$match[1] = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-docs-\2"', $match[1]);
$match[1] = preg_replace('@href="(?:http://flourishlib\.com)?/docs/(f[^"]+)"@', 'href="#\1"', $match[1]);
$match[1] = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[a-zA-Z0-9]+)#([^"]+)"@', 'href="#\1-api-\2"', $match[1]);
$match[1] = preg_replace('@href="(?:http://flourishlib\.com)?/api/(f[^"]+)"@', 'href="#\1-api"', $match[1]);

$toc  = '<p>This document contains all of the class and API documentation, plus selected general documentation for <a href="http://flourishlib.com">Flourish</a> v' . $tag . '. The <em>Class Documentation</em> and <em>General Documentation</em> links in the header provide for quick access to the table of contents. Any links preceeded by a small graphical arrow will leave this document.</p><h2 id="ClassDocumentation">Class Documentation</h2>' . $match[1];
$toc .= '<h2 id="GeneralDocumentation">General Documentation</h2>';

$per_stack = ceil(count($other_docs)/3);
$other_doc_groups = array();
$other_doc_groups[] = array_slice($other_docs, 0, $per_stack);
$other_doc_groups[] = array_slice($other_docs, $per_stack, $per_stack);
$other_doc_groups[] = array_slice($other_docs, $per_stack*2);

foreach ($other_doc_groups as $i => $docs_group) {
	$class = $i ? 'stackable' : 'stackable';
	$toc .= '<ul class="' . $class . '">';
	foreach ($docs_group as $doc) {
		$toc .= '<li><a href="#' . $doc . '-docs">' . $titles[$doc] . '</a></li>';
	}
	$toc .= '</ul>';
}
$toc .= '<div class="clear"></div>';
...
<head> 
	...
	<title>Flourish Offline Documentation (v$tag)</title>
...
$output = preg_replace('@class="((?!ext-link)[^"]+)"( href="[^#][^"]+">)(.*?)</a>@', 'class="\1 ext-link"\2<span class="icon">\3</span></a>', $output);
$output = preg_replace('@(?<!ext-link" )(href="[^#][^"]+">)(.*?)</a>@', 'class="ext-link" \1<span class="icon">\2</span></a>', $output);

$tmp_file = tempnam('/var/tmp', '__floffdoc');
file_put_contents($tmp_file, $output);
//`tidy -i -utf8 -asxhtml --indent-spaces 4 --wrap 0 --escape-cdata 1 --tab-size 4 --vertical-space 0 -f /dev/null -m $tmp_file`;

$output = file_get_contents($tmp_file);
#$output = preg_replace('#^.*<body>#s', '', $output);
#$output = preg_replace('#</body>.*$#s', '', $output);
$output = preg_replace('#(</(li|tr|td)>)\n\n#', "\\1\n", $output);
$output = str_replace(' name="MessagesList-docs-download_template"', '', $output);
unlink($tmp_file);


$output_file = '../flourishlib.com/offline_docs/flourish_v' . $tag . '.html';
file_put_contents($output_file, $header . $output . $footer);

$redirect_file = '../flourishlib.com/offline_docs/index.html';
file_put_contents(
	$redirect_file,
	'<!DOCTYPE html> 
<html>
	<head>
		<title>Redirecting to Latest Offline Docs – Flourish</title>
		<meta http-equiv="refresh" content="0;URL=/offline_docs/flourish_v' . $tag . '.html">
	</head>
	<body>
		<p>
			Redirecting to <a href="/offline_docs/flourish_v' . $tag . '.html">latest offline docs</a>.
		</p>
	</body>
</html>');